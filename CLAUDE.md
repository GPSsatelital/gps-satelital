...
### 📌 SESIÓN 11 JUL 2026 (parte 3) — unificar recepción + permisos detallados (en diseño)
Detalle en memoria [[unificar-recepcion-y-permisos]].
1. **✅ Unificar recepción/retención** (commit `ec9c3bd`, EN PRODUCCIÓN): `MotosView` — botón único **"🏍️ Registrar novedad"** reemplaza "📋 Recepción" + "🚨 Retención"; chooser "¿qué pasó?" que enruta a `ModalRecoleccion` (mora, solo si en mora), recepción→entrega voluntaria, modal de retención legal, `ModalIniciarLiquidacion` (sin bloqueo 7d), o recepción simple. Aditivo (no borró flujos viejos). **Liberar quedó APARTE (Opción B):** botón verde solo si la moto ya está retenida. Texto viejo de Garantía corregido. `tsc`+build limpios. **⚠️ Falta probar en navegador con login.**
2. **🔲 Permisos detallados por usuario — EN DISEÑO, esperando decisión del usuario.** Hoy `profiles.permisos` controla solo MÓDULOS visibles (no acciones). El usuario quiere control de ACCIONES por usuario. **Pregunta abierta SIN responder:** Opción A (catálogo curado de ~10-15 acciones sensibles: registrar efectivo, eliminar pago, crear/editar contrato, iniciar liquidación, dar plazo, recolectar, aprobar visita, etc. — RECOMENDADA) vs Opción B (granular total, no recomendado). Al elegir A: definir catálogo + estructura en permisos + helper `puede(accion)` en AuthContext + reemplazar checks de rol dispersos (mapeo integral) + UI en UsuariosView. Retomar por la respuesta A/B.
