...
| C15 | **Base inicial** | requerida / pagada / falta | `$308.000 + valor del período` contra `ingreso_inicial` + abonos. **`ahorro_apertura` = lo que puso de su base al entrar + lo que ha pagado de su acuerdo de base** (mig 177, disparador `trg_sumar_pago_de_base`; desde la mig 178 todo lo abonado es base, hasta la parte de base del acuerdo). D-023: se le devuelve si se va antes, paga la moto si termina | — |
...
| C18 | **Semanas de más** (D-026, 25-sep-2026) | `semana k de N` · debe para terminar | `semanaDeCierre()` en `cicloPago.ts` ↔ `zala.semana_de_cierre()` (mig 175). Solo cuando **ya pagó todas sus semanas y todavía debe** (deudas + acuerdo). Sigue pagando su **semana normal**, que va entera a lo que debe, hasta quedar en $0. La semana de más *m* vence cuando le tocaría la semana (total + rodadas + *m*) del calendario; si la deuda apareció después de terminar limpio, arrancan desde esa deuda. Se cobran **igual que cualquier semana**: C1, C2 y C5 salen de su semana de más más vieja sin cubrir. En pantalla y en `debe_hoy` el número es **esa semana** (opción A del dueño); lo que debe en total va aparte (`debe_para_terminar`) y **no se suma dos veces**. CESAR y RAMON (más semanas previas que su total) quedan fuera a propósito | `zala.cliente.en_semanas_de_mas` · `semana_de_mas` · `semanas_de_mas` · `debe_para_terminar` |
