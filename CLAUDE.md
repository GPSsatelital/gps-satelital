...
**🔲 PLAN EMPALME/MIGRACIÓN de clientes restantes (COSTA y demás) — EN DEFINICIÓN, retomar preguntas:**
Decisiones ya tomadas con el usuario: ingreso por **SQL en bloque** (como PRADERA/RASTREADOR) · corte **por grupo** (se mantiene `corteMigracionGrupo`) · corte = fecha actual del día de pago de la cuota · **ahorro viejo separado del nuevo mientras el empalme esté abierto** (propuesta aceptada en principio: campo `ahorro_apertura` editable + `ahorro_acumulado` solo con lo nuevo; el display suma ambos) · **al cerrar el empalme se consolidan** (apertura→acumulado, apertura=0, rastro en `contratos_auditoria`) y el indicador de "pendiente de empalme" **desaparece** · deudas de apertura: filas en `deudas`, editables hasta cerrar, bloqueadas al cerrar. **FALTA DEFINIR:** checklist exacta de qué debe confirmarse para poder cerrar el empalme; quién puede cerrar; si hay pantalla de empalme o se maneja desde el Modal Editar Contrato; cómo se ve el indicador de pendiente.
